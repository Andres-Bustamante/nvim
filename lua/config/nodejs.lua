  ...
  if node_path ~= "" then
    ...
    if vim.v.shell_error == 0 then
      ...
      if version then
        ...
        vim.notify("⚠️  Could not parse Node.js version from: " .. version_output .. "\nPlease ensure Node.js is properly installed.", vim.log.levels.ERROR)
        return false, nil
      end
    end
  end

  vim.notify("⚠️  Node.js not found! Some plugins may not work correctly.\nInstall Node.js with:\n  brew install node", vim.log.levels.ERROR)
  return false, nil
end
  ...
  if major_version then
    if major_version >= 18 and major_version < 22 then
      if vim.g.debug_nodejs then
        vim.notify("ℹ️  Node.js v" .. version .. " works but v22+ is recommended for optimal performance.", vim.log.levels.INFO)
      end
    elseif major_version < 18 then
      vim.notify("⚠️  Node.js version " .. version .. " is too old. Neovim requires v18+ (v22+ recommended).", vim.log.levels.WARN)
    end
  end
end
